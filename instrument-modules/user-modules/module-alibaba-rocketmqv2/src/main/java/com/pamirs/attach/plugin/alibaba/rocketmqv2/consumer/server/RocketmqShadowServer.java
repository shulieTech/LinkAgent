...
package com.pamirs.attach.plugin.alibaba.rocketmqv2.consumer.server;
    ...
    public void stop() {
        try {
            ...
            ErrorReporter.buildError()
                    ...
                    .setMessage("Alibaba-RocketMQ消费端关闭失败！")
                    .setDetail("subscription:" + shadowConsumer.getDefaultMQPushConsumerImpl().getSubscriptionInner() + "||"
                            + e.getMessage())
                    .report();
            logger.error("Alibaba-RocketMQ: shutdown shadow DefaultMQPushConsumer err! subscription:{}",
                    shadowConsumer.getDefaultMQPushConsumerImpl().getSubscriptionInner(), e);
        }
    }
}
