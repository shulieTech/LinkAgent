...
/**
 * @ClassName: SaveOperation
 * @author: wangjian
 * @Date: 2020/8/1 00:18
 * @Description:
 */
public class SaveOperation extends AbstractOperation implements Operation {

    @Override
    public Object invoke(Neo4JSessionExt neo4jSession, Object[] args) {
        check(neo4jSession);
        if (args.length > 1) {
            neo4jSession.save(args[0], (Integer) args[1]);
        } else {
            neo4jSession.save(args[0]);
        }
        return null;
    }
}
