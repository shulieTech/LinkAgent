...
public final class RedisUtils {
    ...
    public static void setMaxRedisExpireTime(String methodName, Object[] args) {
        Float maxRedisExpireTime = GlobalConfig.getInstance().getMaxRedisExpireTime();
        try {
            if (maxRedisExpireTime != null && maxRedisExpireTime > -1f) {
                if ((methodName.equals("setex") && args[1] instanceof Long) || (methodName.equals("expire")
                    && args[1] instanceof Long)) {
                    args[1] = Math.min((Long)args[1], Float.valueOf(maxRedisExpireTime * 60 * 60).longValue());
                    ...
                    Method pxMethod;
                    try {
                        exMethod = args[2].getClass().getDeclaredMethod("ex", long.class);
                        pxMethod = args[2].getClass().getDeclaredMethod("px", long.class);
                        if (ex != null && exMethod != null) {
                            exMethod.invoke(args[2], Math.min(ex,
                                Float.valueOf(maxRedisExpireTime * 60 * 60).longValue()));

                        } else if (px != null && pxMethod != null) {
            ...
            LOGGER.error("lettuce maxRedisExpireTime -method: {},new args:{},maxRedisExpireTime:{},exception:{}",methodName,
