...
public class DbcpShadowActiveEventListener implements PradarEventListener {
    ...
    public EventResult onEvent(IEvent event) {
        ...
        if (!(source.getClass().getName().equals("org.apache.commons.dbcp.BasicDataSource"))) {
            return EventResult.IGNORE;
        }
        ...
        try {
            ...
            if (media == null) {
                ...
                return EventResult.success("[dbcp]: handler shadow datasource active event success.");
            }
            // 找到了成对的数据源
            refreshShadowDataSource(dsType, dbcpDataSource, config, media);
            return EventResult.success("[dbcp]: handler shadow datasource active event success.");
        } catch (Exception e) {
            LOGGER.error("[dbcp]: handler shadow datasource active event occur exception", e);
    ...
    private void buildShadowDataSource(int dsType, BasicDataSource dataSource, ShadowDatabaseConfig config) {
        ...
        if (dsType == 1) {
            ...
            if (old != null) {
                LOGGER.info("[dbcp] destroyed shadow table datasource success. url:{} ,username:{}", dataSource.getUrl(), dataSource.getUsername());
                old.close();
            }
            return;
        }
        ...
        if (ptDataSource == null) {
            LOGGER.error("[dbcp] handler shadow datasource active event failed, create shadow datasource error. maybe datasource config is not correct, url: {} username:{} configuration:{}", dataSource.getUrl(), dataSource.getUsername(), config);
    ...
    private void refreshShadowDataSource(int dsType, BasicDataSource dataSource, ShadowDatabaseConfig config, DbcpMediaDataSource media) {
        ...
        if (dsType == 1) {
            ...
            if (ptDataSource != null) {
                ...
                LOGGER.info("[dbcp] biz datasource with url:{}, username:{} change to shadow table type, close shadow datasource!", dataSource.getUrl(), dataSource.getUsername());
            }
            return;
        }
        if (ptDataSource != null) {
        ...
        LOGGER.info("[dbcp] handler shadow datasource active event, refresh shadow datasource, url:{}, username:{}", dataSource.getUrl(), dataSource.getUsername());
