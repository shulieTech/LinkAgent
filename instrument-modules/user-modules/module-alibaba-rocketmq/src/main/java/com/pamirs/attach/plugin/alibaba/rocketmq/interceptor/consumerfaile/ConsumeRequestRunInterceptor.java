...
public class ConsumeRequestRunInterceptor extends AroundInterceptor {
    ...
    public void doBefore(Advice advice) {
        ...
        logger.error("执行ConsumeRequestRunInterceptor before, topic is " + broker + "-" + topic + " queueId is " + queueId);

    }

    @Override
    public void doAfter(Advice advice) {
        logger.error("执行ConsumeRequestRunInterceptor after, topic is " + broker + "-" + topic + " queueId is " + queueId);
    }

    @Override
    public void doException(Advice advice) {
        logger.error("执行ConsumeRequestRunInterceptor error, topic is " + broker + "-" + topic + " queueId is " + queueId
            + " error is "
            + advice.getThrowable().getMessage());
