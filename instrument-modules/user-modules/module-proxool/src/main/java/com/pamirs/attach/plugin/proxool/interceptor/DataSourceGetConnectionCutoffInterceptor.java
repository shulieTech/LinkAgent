...
public class DataSourceGetConnectionCutoffInterceptor extends CutoffInterceptorAdaptor {
    private final static Logger logger = LoggerFactory.getLogger(
            DataSourceGetConnectionCutoffInterceptor.class.getName());
    ...
    public CutOffResult cutoff0(Advice advice) {
        ...
        DataSourceMeta<ProxoolDataSource> dataSourceMeta = new DataSourceMeta<ProxoolDataSource>(
                dataSource.getDriverUrl(), dataSource.getUser(), dataSource);
        ...
        if (DataSourceWrapUtil.pressureDataSources.containsKey(dataSourceMeta)) {
            ProxoolMediaDataSource mediatorDataSource = DataSourceWrapUtil.pressureDataSources.get(dataSourceMeta);
    ...
    private void addListener() {
        ...
        EventRouter.router().addListener(new PradarEventListener() {
            ...
            public EventResult onEvent(IEvent event) {
                ...
                return EventResult.success("proxool-plugin");
            }
            ...
            public EventResult onEvent(IEvent event) {
                ...
                for (ShadowDatabaseConfig config : target) {
                    Iterator<Map.Entry<DataSourceMeta, ProxoolMediaDataSource>> it = DataSourceWrapUtil.pressureDataSources
                            .entrySet().iterator();
                    while (it.hasNext()) {
                        ...
                        if (StringUtils.equalsIgnoreCase(DbUrlUtils.getKey(config.getUrl(), config.getUsername()),
                            ...
                            ProxoolMediaDataSource value = entry.getValue();
                            it.remove();
                            try {
                                value.close();
                                logger.info(
                ...
                return EventResult.success("module-proxool: destroyed shadow table datasource success.");
            }
