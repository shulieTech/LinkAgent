...
public class MongoDB4ShadowPreCheckEventListener implements PradarEventListener {

    private static String mongo_datasource_sync_key = "com.mongodb.client.internal.MongoClientImpl";

    private final static Logger LOGGER = LoggerFactory.getLogger(MongoDB4ShadowPreCheckEventListener.class.getName());
    ...
    private boolean isCollectionValidated(MongoDatabase database, String table, ShadowMongoPreCheckEvent event) {
        try {
            ...
            LOGGER.error("[Mongodb4]: shadow collection {} not exists!", table);
            event.handlerResult(String.format("影子表%s不存在", table));
            return false;
        } catch (Exception e) {
            LOGGER.error("[Mongodb4]: get collection {} occur exception", table, e);
    ...
    private MongoDatabase getDatabase(String url, MongoClient client, ShadowMongoPreCheckEvent event) {
        ...
        if (StringUtils.isEmpty(database)) {
            LOGGER.error("[Mongodb4]: url:{} not assign database", url);
        ...
        try {
            ...
            LOGGER.error("[Mongodb4]: database {} name is illegal", database);
            event.handlerResult(String.format("数据库%s名称不符合规范", database));
        } catch (Exception e) {
            LOGGER.error("[Mongodb4]: get database {} occur exception", database, e);
