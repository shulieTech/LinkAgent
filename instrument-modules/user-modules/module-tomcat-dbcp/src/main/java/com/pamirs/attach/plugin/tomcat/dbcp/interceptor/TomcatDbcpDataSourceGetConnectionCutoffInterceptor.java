...
import com.pamirs.attach.plugin.tomcat.dbcp.util.DbcpMediaDataSource;
...
public class TomcatDbcpDataSourceGetConnectionCutoffInterceptor extends CutoffInterceptorAdaptor {

    private final static Logger logger = LoggerFactory.getLogger(TomcatDbcpDataSourceGetConnectionCutoffInterceptor.class.getName());
    ...
    private void addListener() {
        ...
        EventRouter.router().addListener(new PradarEventListener() {
            ...
            public EventResult onEvent(IEvent event) {
                ...
                return EventResult.success("tomcat-dbcp-plugin");
            }
            ...
            public EventResult onEvent(IEvent event) {
                        ...
                        if (StringUtils.equalsIgnoreCase(DbUrlUtils.getKey(config.getUrl(), config.getUsername()),
                            ...
                            try {
                                ...
                                if (logger.isInfoEnabled()) {
                                    logger.info("module-tomcat-dbcp: destroyed shadow table datasource success. url:{} ,username:{}", entry.getKey().getUrl(), entry.getKey().getUsername());
                                }
                            } catch (Throwable e) {
                                logger.error("module-tomcat-dbcp: closed datasource err! target:{}, url:{} username:{}", entry.getKey().getDataSource().hashCode(), entry.getKey().getUrl(), entry.getKey().getUsername(), e);
                            }
                            break;
                        }
                    }
                ...
                return EventResult.success("module-tomcat-dbcp: destroyed shadow table datasource success.");
            }
